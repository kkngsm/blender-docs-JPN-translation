...
##### Parametric (パラメトリック)
サーフェス上のシェーディング頂点のパラメトリックな座標。エリアライトのために、平面的なマッピングのUV座標とポイントライトへの球面的なマッピングのUV座標を出力する
To area lights it outputs its UV coordinates in planar mapping and in spherical coordinates to point lights.
...
Layer Weight (レイヤーウェイト) ノードは通常 Mix Shader ノードと共にシェーダーの重ねあわせに用いられるウェイトを出力します。
#### 入力
Blend (混合度)
0 から 1 の間で偏りを示す値を出力します。一様でないシェーダーの混合に有用です。
##### Normal (法線)
バンプマップもしくは法線マップを接続する事で出力に影響を与えるための入力です。
##### プロパティ
このノードにプロパティは有りません。
#### 出力
##### Fresnel (フレネル重み値)
